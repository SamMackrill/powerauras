...
-- Create definition.
PowaAuras.UI:Register("LayoutFrame2", {
	Init = function(self, locked, align, pL, pR, pT, pB, mL, mR, mT, mB, debug)
		-- Set me up Scotty.
		self.Columns = {};
		self.Items = {};
		self.Debug = debug or false;
		self.Align = align or self.AlignMode.Left;
		...
		if(parent and parent:IsObjectType("ScrollFrame")) then
			self.IsScrollChild = true;
		end
		self:UpdateLayout();
	end,
	AlignMode = {
	...
	ResizeMode = {
		Always = 0x1, -- Forces element size to adapt.
		Needed = 0x2, -- Resizes if the element is larger than the effective column width.
		Never = 0x4, -- No resizing of items is performed.
	},
	AddColumn = function(self, width, height, pL, pR, pT, pB, mL, mR, mT, mB, align, resize)
		tinsert(self.Columns, {
			Align = align or self.AlignMode.Left,
			Resize = resize or self.ResizeMode.Needed,
			Width = width or 0,
			Height = height or 0,
			Padding = {
				Left = pL or 0,
	...
	AddItem = function(self, item, colspan, pL, pR, pT, pB, mL, mR, mT, mB, align)
		tinsert(self.Items, {
			Align = align or self.AlignMode.Left,
			Columns = colspan,
			Frame = item,
	...
	GetEffectiveGridWidth = function(self)	
	end,
	...
	UpdateLayout = function(self)
		...
		local i, items, item, column, row, cOffset = 1, #(self.Items), nil, 0, 1, 1;
		for i=1, items do
			-- Update column/row data.
		end
	end,
});