	...
	frame.UpdateLayout = function(self)
		...
		for i=1,itemCount do
			...
			if(column+(item.LayoutOpts["Columns"]-1) <= self.Columns) then
				for j=column, column+(item.LayoutOpts["Columns"]-1) do
					cW = cW + (self.ColumnSizes[j]["X"] or 0);
					cH = (cH > (self.ColumnSizes[j]["Y"] or 0) and cH or (self.ColumnSizes[j]["Y"] or 0));
				end
			else
				cW = (self.ColumnSizes[column]["X"] or 0);
				cH = (self.ColumnSizes[column]["Y"] or 0);
			end
		...
		for i=1, 9 do
			...
			if(i == 1) then
				...
				PowaAuras.UI.Tooltip(frame, item:GetName(), "Width: " .. fcW .. "\nHeight: " .. fcH .. 
					...
					"\n\nPadding (L): " .. pL .. "\nPadding (R): " .. pR .. "\nPadding (T): " .. pT .. "\nPadding (B): " .. pB .. 
					"\n\nMargin (L): " .. mL .. "\nMargin (R): " .. mR .. "\nMargin (T): " .. mT .. "\nMargin (B): " .. mB .. 
					"\n\nOffset (X): " .. oX .. "\nOffset (Y): " .. oY .. "\nDisplayed Offset (X): " .. oX+pL+mL .. "\nDisplayed Offset (Y): " .. oY-pT-mT);
