...
function assertEquals(actual, expected, assertName, tol)
	-- assert that two values are equal and calls error else
	local compare = false;
	if (type(actual)=="number" and type(expected)=="number") then
		local actualInt = math.ceil(actual);
		local expectedInt = math.ceil(expected);
		if (actualInt == expectedInt) then
			local diff = actual - expected;
			if (tol==nil) then tol = 0.0001; end
			compare = (diff < tol);
		end
	else
	...
	if  not compare  then
		...
		local errorMsg
		local errorPrefix = "expected";
		if (assertName~=nil) then
			errorPrefix = assertName.." "..errorPrefix;
		end
		if type(expected) == 'string' then
			errorMsg = "\n"..errorPrefix..": "..wrapValue(expected).."\n"..
                             "actual  : "..wrapValue(actual).."\n"
		else
			errorMsg = errorPrefix..": "..wrapValue(expected)..", actual: "..wrapValue(actual)
		end
		--print (errorMsg)
		error( errorMsg, 2 )
	end
